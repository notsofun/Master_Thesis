# python unsupervised_classification/bertopic.py
import os
import logging
import warnings
import pandas as pd
...
def setup_logger():
    os.makedirs('unsupervised_classification/logs', exist_ok=True)
    log_filename = datetime.now().strftime("logs/topic_pipeline_%Y%m%d_%H%M.log")
...
def run_topic_modeling_pipeline(langs_to_analyze):
    ...
    logger.info(f"我们发现了{len(set(hdbscan_model.labels_))}个簇")
