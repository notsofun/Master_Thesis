...
class HatePipeline:
    def __init__(self, logger, input_csv: str, model_specs: List[Dict], gemini_api_key: str,
    ...
    def run_detection(self):
        ...
        # ───────── 2. 再对抽样文本做 HS 检测 ─────────
        detector = HateSpeechDetector(self.logger, self.models, device=self.device)
        ...
        stats = {}
        # tqdm 包围循环
        for name, df_model in tqdm(sample_results.items(), desc="Stats", ncols=100):

            df_sample = df_model.iloc[idxs].reset_index(drop=True)
            hs_labels = df_sample["HS"].fillna(0).astype(int).tolist()
            gem_labels = [1 if (g.get("is_hate") is True) else 0 for g in gemini_results]
            total = len(hs_labels)
        ...
        self.logger.info(f"Model accuracies vs Gemini: { {k:v['accuracy'] for k,v in stats.items()} }")
        self.logger.info(f"Selected best model: {best_name} with accuracy {best_stats['accuracy']:.4f}")

        # ───────── 6. 生成待标注 CSV ─────────
        rows = []
        df_best = sample_results[best_name].reset_index(drop=True)
        for i, idx in enumerate(idxs):
            text = sampled_texts[i]
            hs_label = int(df_best.iloc[idxs[i]]["HS"]) if "HS" in df_best.columns else None
            gem_label = 1 if (gemini_results[i].get("is_hate") is True) else 0
            rows.append({
                "text": text,
        ...
        out_csv = os.path.join(self.output_dir, f"to_annotate_{best_name}.csv")
