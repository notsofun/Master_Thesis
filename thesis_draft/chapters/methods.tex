...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners=3pt, draw=black, fill=gray!10,
              text width=5.8cm, align=center, minimum height=0.8cm, font=\small},
  laybox/.style={rectangle, rounded corners=3pt, draw=black, fill=white,
                 text width=5.8cm, align=center, minimum height=0.8cm, font=\small},
  arr/.style={-{Stealth[length=5pt]}, thick}
]
...
\node[laybox, below=of inp]  (l1)  {Layer 1: Dependency parsing $\rightarrow$ SVO triples};
\node[laybox, below=of l1]   (l2)  {Layer 2: Predicate window ($\pm N$ words around target)};
\node[box,    below=of l2]   (l3)  {Layer 3: Gemini LLM $\rightarrow$ 10-class frame label};

\node[box, below=of l3] (tax) {Attack frame taxonomy (9 substantive + \textit{other})};
...
% bracket label for layers
\node[font=\footnotesize\itshape, text=gray, right=0.3cm of l1] {complementary};
\node[font=\footnotesize\itshape, text=gray, right=0.3cm of l2] {for zero-subject sentences};

\end{tikzpicture}
\caption{RQ2 frame classification pipeline. Layers 1--2 extract predicate--argument structure using complementary syntactic methods; Layer 3 maps these to a theoretically grounded attack frame taxonomy via LLM.}
...
\node[dictbox, right=0.3cm of mfd]  (jmfd) {J-MFD\\(JP)};
\node[dictbox, right=0.3cm of jmfd] (cmfd) {CMFD\\(ZH)};
\node[dictbox, right=0.3cm of cmfd] (cust) {Custom\\Threat};

\node[box, below=0.6cm of jmfd, xshift=0.6cm] (axis)
...
\draw[darr] (jmfd.south) -- (axis.north);
\draw[darr] (cmfd.south) |- (axis.east);
